...
import math
ham_chucnang =[
            "list(enumerate(iterable)) — ép thành list các cặp (index,value) — list(enumerate(['a','b'])) → [(0,'a'),(1,'b')].",
            "tuple(enumerate(iterable)) — ép thành tuple — tuple(enumerate('ab')) → ((0,'a'),(1,'b')).",
            "dict(enumerate(iterable)) — biến thành dict với index làm key — dict(enumerate(['x','y'])) → {0:'x',1:'y'}.",
            "for i,v in enumerate(iterable, start) — duyệt vừa có index vừa có value — for i,v in enumerate(['a','b'],1) → (1,'a'), (2,'b').",
            "max(enumerate(iterable), key=lambda x:x[1]) — tìm phần tử lớn nhất kèm index — max(enumerate([3,1,5]), key=lambda x:x[1]) → (2,5).",
            "min(enumerate(iterable), key=lambda x:x[1]) — tìm phần tử nhỏ nhất kèm index.",
            "sorted(enumerate(iterable), key=lambda x:x[1]) — sắp xếp kèm index."
            ]

baitap = """- Nhấm phím 1 để làm bài tập 1: Tìm vị trí phần tử lặp đầu tiên trong danh sách
- Nhấm phím 2 để làm bài tập 2: So sánh 2 file d liệu để phát hiện dòng khác nhau
- Nhấm phím Enter để quay lại menu chính
- Nhấn phím khác để thoát."""

def show():
    print(f"\n=== Chào mừng bạn đến với enumerate (đánh số khi lặp) ===")
    print("• Cú pháp: enumerate(iterable, start=0), for i,v in enumerate(lst)")
    print("• Cấu trúc: Sinh cặp (index, value).")
    print("• Áp dụng: Duyệt cùng lúc chỉ số & giá trị.")
    print("• Note: start có thể tuỳ chỉnh.")
    print("• Hàm/phương thức thường gặp:")
    for f in ham_chucnang:
        print("   -", f)
    print(f"""\nChúc mừng bạn vừa tìm hiểu xong về cấu trúc enumerate!
